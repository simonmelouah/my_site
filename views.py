...
@app.route('/software-portfolio', methods=['GET', 'POST'])  # pragma: no cover
def software_portfolio():
    ...
    if "data-hover" in project_dict:
        ...
        connect.add_project_tracking(
            project_dict["data-click-git"], "click-git")
    elif "data-click-youtube" in project_dict:
        connect.add_project_tracking(
            project_dict["data-click-youtube"],
            "click-youtube")
    connect.close_connection()
...
@app.route('/add-project', methods=['GET', 'POST'])  # pragma: no cover
    ...
    for i in project_stats:
        if i.hovers > 0 or i.git_clicks > 0 or i.youtube_clicks > 0:
            slack_string += "Project: {0} \nHovers: {1} \nGit-Clicks: {2} \nYoutube-Clicks: {3} \n\n".format(
                i.title, i.hovers, i.git_clicks, i.youtube_clicks)
