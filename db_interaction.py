...
class DbInteraction(object):
    ...
    def get_user(self, username):
        try:
            login = self.db_session.query(User).filter(
        ...
        try:
            if technology_id:
                case_conditions = case(
                    [(Technology.id == technology_id, None)], else_=Technology.name).label("case_condition")
                technologies = self.db_session.query(
                    Technology).order_by(case_conditions.asc()).all()
            else:
                technologies = self.db_session.query(
                    Technology).order_by(Technology.name.asc()).all()
            return technologies
        except:
            self.db_session.rollback()
            raise
    ...
    def category_choices(self, category_id=None):
        ...
        try:
            if category_id:
                ...
                case_conditions = case(
                    [(Category.id == category_id, None)], else_=Category.name).label("case_condition")
                categories = self.db_session.query(
                    Category).order_by(case_conditions.asc()).all()
            else:
                categories = self.db_session.query(
                    Category).order_by(Category.name.asc()).all()
            return categories
        except:
            self.db_session.rollback()
            raise
    ...
    def get_technology(self, technology_name):
        ...
        try:
            technology = self.db_session.query(Technology).filter(
    ...
    def project_tracking(self):
        try:
            ...
            get_project_stats = self.db_session.query(Project.title.label('title'),
                                                      func.count(hovers.id.distinct()).label(
                                                          'hovers'),
                                                      func.count(git_clicks.id.distinct()).label(
                                                          'git_clicks'),
